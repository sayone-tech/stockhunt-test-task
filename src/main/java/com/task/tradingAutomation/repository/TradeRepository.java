...
public interface TradeRepository extends JpaRepository<Trades, Long> {
    ...
    @Query("SELECT t FROM Trades t WHERE t.status = :status AND t.orderType = :orderType")
    List<Trades> findByStatusAndOrderType(@Param("status") TradeStatus status, @Param("orderType") DhanOrderRequest.OrderType orderType);

    @Query("SELECT t FROM Trades t WHERE t.orderId = :orderId")
    Trades findByOrderId(@Param("orderId") String orderId);
}
